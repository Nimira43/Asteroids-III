...
class Bullet(pygame.sprite.Sprite):
    def __init__(self, idx, position, config):
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load(config.IMAGEPATHS['bullet']).convert_alpha()
...
class Asteroid(pygame.sprite.Sprite):
    def __init__(self, config):
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load(config.IMAGEPATHS['asteroid']).convert_alpha()
        self.rect = self.image.get_rect()
        self.position = (random.randrange(20, config.SCREENSIZE[0] - 20), -64)
        self.rect.left, self.rect.top = self.position 
...
class Ship(pygame.sprite.Sprite):
    ...
    def draw(self, screen):
        screen.blit(self.image, self.rect)
    def shot(self):
        return Bullet(self.player_idx, (self.rect.center[0] - 5, self.position['y'] - 5), self.config)
